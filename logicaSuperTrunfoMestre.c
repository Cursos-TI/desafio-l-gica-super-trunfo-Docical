#include <stdio.h>

int main(){
    char estado[20], code[5], cidade[50];
    int populacao, PontosTuristicos;
    float area, pib, superpoder1;

    char estado2[20], code2[5], cidade2[50];
    int populacao2, PontosTuristicos2;
    float area2, pib2, superpoder2;

    int opcao, opcao2, opcao3;
    int carta1o, carta2o;
    int carta1o2, carta2o2;
    float carta1, carta2;

    //Menu
    printf("---Cartas Super Trunfo---\n\n");
    printf("1 - Comecar Jogo\n");
    printf("2 - Regras do jogo\n");
    printf("3 - Sair do Jogo\n");
    printf("Digite uma opcao: ");
    scanf("%d", &opcao);

    // Carta 1
    switch(opcao){
    case 1:
        printf("Escreva o Estado da Carta 1: ");
        scanf(" %[^\n]", estado);       //utilizei o  "%[^\n]"ao inves de "%s" pra ele conseguir ler os espaços
        printf("Agora, escreva o codigo da Carta 1 (ex: A01, B02): ");
        scanf("%s", code);
        printf("Qual o nome da cidade da Carta 1? ");
        scanf(" %[^\n]", cidade);
        printf("me fale o numero da populacao da Carta 1: ");
        scanf("%d", &populacao);
        printf("Digite a Area da Carta 1: ");
        scanf("%f", &area);
        printf("Digite o PIB: ");
        scanf("%f", &pib);
        printf("Qual a quantidade de Pontos Turisticos da Carta 1? ");
        scanf("%d", &PontosTuristicos);

        // Carta 2
        printf("\nEscreva o Estado da Carta 2: ");
        scanf(" %[^\n]", estado2);
        printf("Agora, escreva o codigo da Carta 2: ");
        scanf("%s", code2);
        printf("Qual o nome da cidade da Carta 2? ");
        scanf(" %[^\n]", cidade2);
        printf("me fale o numero da populacao da Carta 2: ");
        scanf("%d", &populacao2);
        printf("Digite a Area da Carta 2: ");
        scanf("%f", &area2);
        printf("Digite o PIB: ");
        scanf("%f", &pib2);
        printf("Qual a quantidade de Pontos Turisticos da Carta 2? ");
        scanf("%d", &PontosTuristicos2);

        //Densidade Populacional e PIB per Capita
        float densidadep = (float) populacao / area;
        float densidadep2 = (float) populacao2 / area2;

        float pibc = (pib * 1000000000) / (float) populacao;
        float pibc2 = (pib2 * 1000000000) / (float) populacao2;

        // Super Poder
        superpoder1 = populacao + area + pib + pibc + (1 / densidadep) + PontosTuristicos;
        superpoder2 = populacao2 + area2 + pib2 + pibc2 + (1 / densidadep2) + PontosTuristicos2;

        //Resultado Carta 1
        printf("\n-Passando as Informacoes das Cartas-\n\n-----CARTA 1-----\n");
        printf("Estado: %s", estado);
        printf("\nCodigo: %s\n", code);
        printf("Cidade: %s\n", cidade);
        printf("Populacao: %d\n", populacao);
        printf("Area: %.2fkm2\n", area);
        printf("Densidade Populacional: %.2fhab/km2\n", densidadep);
        printf("PIB: %.2f Bilhoes de Reais\n", pib);
        printf("PIB per Capita: %.2f reais\n", pibc);
        printf("Numero de Pontos Turisticos: %d\n", PontosTuristicos);
        printf("Super Poder: %.2f\n", superpoder1);

        //Resultado Carta 2
        printf("\n-----CARTA 2-----\n");
        printf("Estado: %s\n", estado2);
        printf("Codigo: %s\n", code2);
        printf("Cidade: %s\n", cidade2);
        printf("Populacao: %d\n", populacao2);
        printf("Area: %.2fkm2\n", area2);
        printf("Densidade Populacional: %.2fhab/km\n", densidadep2);
        printf("PIB: %.2f Bilhoes de Reais\n", pib2);
        printf("PIB per Capita: %.2f reais\n", pibc2);
        printf("Numero de Pontos Turisticos: %d\n", PontosTuristicos2);
        printf("Super Poder: %.2f\n", superpoder2);

        printf("\n-----COMPARANDO CARTAS-----\n");
        printf("1 - Populacao\n");
        printf("2 - Area\n");
        printf("3 - Pontos Turisticos\n");
        printf("4 - PIB\n");
        printf("5 - Densidade Populacional\n");
        printf("6 - PIB Per Capita\n");
        printf("7 - Super Poder\n");
        printf("Digite o Atributo que voce quer comparar: ");
        scanf("%d", &opcao2);
        printf("Digite outro Atributo que voce deseja comparar: ");
        scanf("%d", &opcao3);

        if (opcao2 < 1 || opcao2 > 7 || opcao3 < 1 || opcao3 > 7) {
            printf("Um dos atributos escolhidos é invalido");

        }

        if (opcao2 == opcao3){
            printf("Os atributos escolhidos devem ser diferentes");
    
        }

        switch(opcao2) {
            case 1:
            carta1o = populacao;
            carta2o = populacao2;
            printf("\n---Comparando Populacao---\n");
            printf("Carta 1 - %s, contem uma populacao de %d\n", cidade, populacao);
            printf("Carta 2 - %s, contem uma populacao de %d\n", cidade2, populacao2);
            if (populacao > populacao2) {
                printf("Populacao: Carta 1 - %s Ganhou!!\n", cidade);
            } else if(populacao < populacao2) {
                printf("Populacao: Carta 2 - %s Ganhou!!\n", cidade2);
            } else {
                printf("Populacao: Houve um Empate!!\n");
            }
            break;

            case 2:
            carta1o = area;
            carta2o = area2;
            printf("\n---Comparando Area---\n");
            printf("Carta 1 - %s, contem uma area de %.2f\n", cidade, area);
            printf("Carta 2 - %s, contem uma area de %.2f\n", cidade2, area2);
            if (area > area2) {
                printf("Area: Carta 1 - %s Ganhou!!\n", cidade);
            } else if(area < area2) {
                printf("Area: Carta 2 - %s Ganhou!!\n", cidade2);
            } else {
                printf("Area: Houve um Empate!!\n");
            }
            break;

            case 3:
            carta1o = PontosTuristicos;
            carta2o = PontosTuristicos2;
            printf("\n---Comparando Pontos Turisticos---\n");
            printf("Carta 1 - %s, contem um valor de %d\n", cidade, PontosTuristicos);
            printf("Carta 2 - %s, contem um valor de %d\n", cidade2, PontosTuristicos2);
            if (PontosTuristicos > PontosTuristicos2) {
                printf("Pontos Turisticos: Carta 1 - %s Ganhou!!\n", cidade);
            } else if (PontosTuristicos < PontosTuristicos2) {
                printf("Pontos Turisticos: Carta 2 - %s Ganhou!!\n", cidade2);
            } else {
                printf("Pontos Turisticos: Houve um Empate!!\n");
            }
            break;

            case 4:
            carta1o = pib;
            carta2o = pib2;
            printf("\n---Comparando PIB---\n");
            printf("Carta 1 - %s, contem um PIB de %.2f\n", cidade, pib);
            printf("Carta 2 - %s, contem um PIB de %.2f\n", cidade2, pib2);
            if (pib > pib2) {
                printf("PIB: Carta 1 - %s Ganhou!!\n", cidade);
            } else if(pib < pib2) {
                printf("PIB: Carta 2 - %s Ganhou!!\n", cidade2);
            } else {
                printf("PIB: Houve um Empate!!\n");
            }
            break;

            case 5:
            carta1o = densidadep;
            carta2o = densidadep2;
            printf("\n---Comparando Densidade Populacional---\n");
            printf("Carta 1 - %s, contem uma Densidade Populacional de %.2f\n", cidade, densidadep);
            printf("Carta 2 - %s, contem uma Densidade Populacional de %.2f\n", cidade2, densidadep2);
            if (densidadep < densidadep2) {
                printf("Densidade Populacional: Carta 1 - %s Ganhou!!\n", cidade);
            } else if(densidadep > densidadep2) {
                printf("Densidade Populacional: Carta 2 - %s Ganhou!!\n", cidade2);
            } else {
                printf("Densidade Populacional: Houve um Empate!!\n");
            }
            break;

            case 6:
            carta1o = pibc;
            carta2o = pibc2;
            printf("\n---Comparando PIB Per Capita---\n");
            printf("Carta 1 - %s, contem um PIB Per Capita de %.2f\n", cidade, pibc);
            printf("Carta 2 - %s, contem um PIB Per Capita de %.2f\n", cidade2, pibc2);
            if (pibc > pibc2) {
                printf("PIB Per Capita: Carta 1 - %s Ganhou!!\n", cidade);
            } else if(pibc < pibc2) {
                printf("PIB Per Capita: Carta 2 - %s Ganhou!!\n", cidade2);
            } else {
                printf("PIB Per Capita: Houve um Empate!!\n");
            }
            break;

            case 7:
            carta1o = superpoder1;
            carta2o = superpoder2;
                printf("\n---Comparando Super Poder---\n");
                printf("Carta 1 - %s, contem um Super Poder de %.2f\n", cidade, superpoder1);
                printf("Carta 2 - %s, contem um Super Poder de %.2f\n", cidade2, superpoder2);
            if (superpoder1 > superpoder2) {
                printf("Super Poder: Carta 1 - %s Ganhou!!\n", cidade);
            } else if(superpoder1 < superpoder2) {
                printf("Super Poder: Carta 2 - %s Ganhou!!\n", cidade2);
            } else {
                printf("Super Poder: Houve um Empate!!\n");
            }
            break;
            default:
                printf("Opcao Invalida. Tente Novamente");
        }
            switch(opcao3) {
                    case 1:
                    carta1o2 = populacao;
                    carta2o2 = populacao2;
                    printf("\n---Comparando Populacao---\n");
                    printf("Carta 1 - %s, contem uma populacao de %d\n", cidade, populacao);
                    printf("Carta 2 - %s, contem uma populacao de %d\n", cidade2, populacao2);
                    if (populacao > populacao2) {
                        printf("Populacao: Carta 1 - %s Ganhou!!\n\n", cidade);
                    } else if(populacao < populacao2) {
                        printf("Populacao: Carta 2 - %s Ganhou!!\n\n", cidade2);
                    } else {
                        printf("Populacao: Houve um Empate!!\n\n");
                    }
                    carta1 = carta1o + carta1o2;
                    carta2 = carta2o + carta2o2;
                            
                    printf("Somando os Atributos Escolhidos...\n");
                    printf("O Valor da soma dos atributos escolhido na Carta 1 e:%.2f\n", carta1);
                    printf("O Valor da soma dos atributos escolhido na Carta 2 e:%.2f\n", carta2);
                    if (carta1 > carta2){
                         printf("Carta 1 GANHOU!!");
                        } else if(carta1 < carta2) {
                           printf("Carta 2 GANHOU!!");
                         } else {
                           printf("Houvemos um Empate!!");
                         }
                    break;
        
                    case 2:
                    carta1o2 = area;
                    carta2o2 = area2;
                    printf("\n---Comparando Area---\n");
                    printf("Carta 1 - %s, contem uma area de %.2f\n", cidade, area);
                    printf("Carta 2 - %s, contem uma area de %.2f\n", cidade2, area2);
                    if (area > area2) {
                        printf("Area: Carta 1 - %s Ganhou!!\n\n", cidade);
                    } else if(area < area2) {
                        printf("Area: Carta 2 - %s Ganhou!!\n\n", cidade2);
                    } else {
                        printf("Area: Houve um Empate!!\n\n");
                    }
                    carta1 = carta1o + carta1o2;
                    carta2 = carta2o + carta2o2;
                            
                    printf("Somando os Atributos Escolhidos...\n");
                    printf("O Valor da soma dos atributos escolhido na Carta 1 e:%.2f\n", carta1);
                    printf("O Valor da soma dos atributos escolhido na Carta 2 e:%.2f\n", carta2);
                    if (carta1 > carta2){
                         printf("Carta 1 GANHOU!!");
                        } else if(carta1 < carta2) {
                           printf("Carta 2 GANHOU!!");
                         } else {
                           printf("Houvemos um Empate!!");
                         }
                    break;
                    case 3:
                    carta1o2 = PontosTuristicos;
                    carta2o2 = PontosTuristicos2;
                    printf("\n---Comparando Pontos Turisticos---\n");
                    printf("Carta 1 - %s, contem um valor de %d\n", cidade, PontosTuristicos);
                    printf("Carta 2 - %s, contem um valor de %d\n", cidade2, PontosTuristicos2);
                    if (PontosTuristicos > PontosTuristicos2) {
                        printf("Pontos Turisticos: Carta 1 - %s Ganhou!!\n\n", cidade);
                    } else if (PontosTuristicos < PontosTuristicos2) {
                        printf("Pontos Turisticos: Carta 2 - %s Ganhou!!\n\n", cidade2);
                    } else {
                        printf("Pontos Turisticos: Houve um Empate!!\n\n");
                    }
                    carta1 = carta1o + carta1o2;
                    carta2 = carta2o + carta2o2;
                            
                    printf("Somando os Atributos Escolhidos...\n");
                    printf("O Valor da soma dos atributos escolhido na Carta 1 e:%.2f\n", carta1);
                    printf("O Valor da soma dos atributos escolhido na Carta 2 e:%.2f\n", carta2);
                    if (carta1 > carta2){
                         printf("Carta 1 GANHOU!!");
                        } else if(carta1 < carta2) {
                           printf("Carta 2 GANHOU!!");
                         } else {
                           printf("Houvemos um Empate!!");
                         }
                    break;
        
                    case 4:
                    carta1o2 = pib;
                    carta2o2 = pib2;
                    printf("\n---Comparando PIB---\n");
                    printf("Carta 1 - %s, contem um PIB de %.2f\n", cidade, pib);
                    printf("Carta 2 - %s, contem um PIB de %.2f\n", cidade2, pib2);
                    if (pib > pib2) {
                        printf("PIB: Carta 1 - %s Ganhou!!\n\n", cidade);
                    } else if(pib < pib2) {
                        printf("PIB: Carta 2 - %s Ganhou!!\n\n", cidade2);
                    } else {
                        printf("PIB: Houve um Empate!!\n\n");
                    }
                    carta1 = carta1o + carta1o2;
                    carta2 = carta2o + carta2o2;
                            
                    printf("Somando os Atributos Escolhidos...\n");
                    printf("O Valor da soma dos atributos escolhido na Carta 1 e:%.2f\n", carta1);
                    printf("O Valor da soma dos atributos escolhido na Carta 2 e:%.2f\n", carta2);
                    if (carta1 > carta2){
                         printf("Carta 1 GANHOU!!");
                        } else if(carta1 < carta2) {
                           printf("Carta 2 GANHOU!!");
                         } else {
                           printf("Houvemos um Empate!!");
                         }
                    break;
        
                    case 5:
                    carta1o2 = densidadep;
                    carta2o2 = densidadep2;
                    printf("\n---Comparando Densidade Populacional---\n");
                    printf("Carta 1 - %s, contem uma Densidade Populacional de %.2f\n", cidade, densidadep);
                    printf("Carta 2 - %s, contem uma Densidade Populacional de %.2f\n", cidade2, densidadep2);
                    if (densidadep < densidadep2) {
                        printf("Densidade Populacional: Carta 1 - %s Ganhou!!\n\n", cidade);
                    } else if(densidadep > densidadep2) {
                        printf("Densidade Populacional: Carta 2 - %s Ganhou!!\n\n", cidade2);
                    } else {
                        printf("Densidade Populacional: Houve um Empate!!\n\n");
                    }
                    carta1 = carta1o + carta1o2;
                    carta2 = carta2o + carta2o2;
                            
                    printf("Somando os Atributos Escolhidos...\n");
                    printf("O Valor da soma dos atributos escolhido na Carta 1 e:%.2f\n", carta1);
                    printf("O Valor da soma dos atributos escolhido na Carta 2 e:%.2f\n", carta2);
                    if (carta1 > carta2){
                         printf("Carta 1 GANHOU!!");
                        } else if(carta1 < carta2) {
                           printf("Carta 2 GANHOU!!");
                         } else {
                           printf("Houvemos um Empate!!");
                         }
                    break;
        
                    case 6:
                    carta1o2 = pibc;
                    carta2o2 = pibc2;
                    printf("\n---Comparando PIB Per Capita---\n");
                    printf("Carta 1 - %s, contem um PIB Per Capita de %.2f\n", cidade, pibc);
                    printf("Carta 2 - %s, contem um PIB Per Capita de %.2f\n", cidade2, pibc2);
                    if (pibc > pibc2) {
                        printf("PIB Per Capita: Carta 1 - %s Ganhou!!\n\n", cidade);
                    } else if(pibc < pibc2) {
                        printf("PIB Per Capita: Carta 2 - %s Ganhou!!\n\n", cidade2);
                    } else {
                        printf("PIB Per Capita: Houve um Empate!!\n\n");
                    }
                    carta1 = carta1o + carta1o2;
                    carta2 = carta2o + carta2o2;
                            
                    printf("Somando os Atributos Escolhidos...\n");
                    printf("O Valor da soma dos atributos escolhido na Carta 1 e:%.2f\n", carta1);
                    printf("O Valor da soma dos atributos escolhido na Carta 2 e:%.2f\n", carta2);
                    if (carta1 > carta2){
                         printf("Carta 1 GANHOU!!");
                        } else if(carta1 < carta2) {
                           printf("Carta 2 GANHOU!!");
                         } else {
                           printf("Houvemos um Empate!!");
                         }
                    break;
        
                    case 7:
                    carta1o2 = superpoder1;
                    carta2o2 = superpoder2;
                        printf("\n---Comparando Super Poder---\n");
                        printf("Carta 1 - %s, contem um Super Poder de %.2f\n", cidade, superpoder1);
                        printf("Carta 2 - %s, contem um Super Poder de %.2f\n", cidade2, superpoder2);
                    if (superpoder1 > superpoder2) {
                        printf("Super Poder: Carta 1 - %s Ganhou!!\n\n", cidade);
                    } else if(superpoder1 < superpoder2) {
                        printf("Super Poder: Carta 2 - %s Ganhou!!\n\n", cidade2);
                    } else {
                        printf("Super Poder: Houve um Empate!!\n\n");
                    }   
                    carta1 = carta1o + carta1o2;
                    carta2 = carta2o + carta2o2;
                            
                    printf("Somando os Atributos Escolhidos...\n");
                    printf("O Valor da soma dos atributos escolhido na Carta 1 e:%.2f\n", carta1);
                    printf("O Valor da soma dos atributos escolhido na Carta 2 e:%.2f\n", carta2);
                    if (carta1 > carta2){
                         printf("Carta 1 GANHOU!!");
                        } else if(carta1 < carta2) {
                           printf("Carta 2 GANHOU!!");
                         } else {
                           printf("Houvemos um Empate!!");
                         }
                    break;
                    default:
                     printf("Opcao Invalida. Tente Novamente");
                        }
                                        
                    
        break;
    case 2:
        printf("As Regras do Jogo sao...\n");
        printf("vence a carta com o maior valor no atributo escolhido.\nPorem, para a Densidade Demografica, a regra inverte: vence a carta com o menor valor.");
        break;
    case 3:
        printf("Fechando Jogo...");
        break;
    default:
        printf("Opcao Invalida. Tente Novamente.");
        return 0;
    }
}   